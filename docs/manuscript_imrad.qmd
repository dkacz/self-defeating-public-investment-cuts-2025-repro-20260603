...
format:
  ...
  pdf:
    pdf-engine: lualatex
    include-in-header: formatting/professional_pdf_header_20260609.tex
---

# Abstract

EU fiscal surveillance evaluates national budgets through debt projections and net expenditure ceilings. These assessments translate assumptions about growth, interest rates, fiscal multipliers, and adjustment composition into judgements about fiscal risk and required consolidation. Public investment is directly exposed to this logic because capital projects are often easier to postpone than it is to raise taxes or reduce transfers, public wages, or entitlements. The analysis asks how estimated public investment responses change the debt implications of Polish investment policy. The empirical model estimates local projections for public investment shocks on an annual EU27 panel covering the period from the EU's 2004 enlargement through 2025. The resulting output and spending paths enter two debt calculations: an institutional debt equation using the Commission baseline and a direct debt to GDP local projection path.

These paths evaluate a consolidation scenario in which annual public investment is reduced by one percentage point of GDP in each year from 2028 to 2030. To avoid treating Poland as identical to the panel average, the specifications evaluate Polish values of investment import content, household liquidity position, public debt, and real GDP per capita in PPS; the debt scenario carries the three channels that satisfy the retention criterion specified in advance. By 2036, the cut raises the debt to GDP ratio by 3.7 to 7.3 percentage points across the retained Polish channel evaluations under the institutional debt equation, and by 1.7 to 3.5 percentage points under the direct debt to GDP path. The mechanism is self defeating: the discretionary primary balance gain from cutting investment is outweighed by persistent output losses, related cyclical primary balance deterioration through automatic stabilisers, and the denominator effect on the debt ratio.

# 1 Introduction
## 1.1 EU fiscal surveillance and debt sustainability analysis

EU fiscal surveillance evaluates member states' fiscal plans by applying legal rules, numerical indicators, medium term adjustment trajectories, and model based assessments within the Stability and Growth Pact and the European Semester. This framework sets the institutional context in which the European Commission and the Council measure fiscal efforts and scrutinise national budgetary decisions (Schmidt, 2015; Van der Veer, 2022; European Commission, 2026). Recent reforms to the EU fiscal framework strengthen the role of debt sustainability analysis (Heimberger et al., 2024). Within the reformed framework, the Commission uses debt sustainability analysis in prior guidance, assessment of medium term budgetary plans, bilateral discussions with member states, and corrective trajectories where fiscal risks are considered significant (European Commission, 2026).
...
Such caution also applies to broad evidence on fiscal consolidation. In the expansionary austerity literature, multi year fiscal plans are often classified by their predominant tax or expenditure component, and subsequent macroeconomic outcomes are then read as evidence on the relative effects of those classes of adjustment (Alesina and Ardagna, 2010; Alesina, Favero and Giavazzi, 2015). Aggregate plans differ from fiscal instruments, and plan labels differ from shocks. A plan classified as expenditure based can combine public consumption, transfers, public investment, procurement deferral, tax measures, and implementation changes. When consolidation is adopted under debt stress, weak growth, market pressure, or external surveillance, the conditions that select a country into treatment can also shape the macroeconomic path that follows. Guajardo, Leigh and Pescatori (2014) show that replacing conventional cyclically adjusted primary balance measures, abbreviated CAPB, with narratively identified deficit driven consolidation episodes reverses the main expansionary austerity conclusion and yields contractionary effects on private domestic demand and GDP. Jorda and Taylor (2016) reach a similar reversal when the non random allocation of consolidation episodes across economic conditions is corrected with propensity score methods. Adler, Allen, Ganelli and Leigh (2024) extend the action based dataset and restate the same identification concern: CAPB changes may reflect cyclical and asset price movements, or policy responses to prospective conditions, rather than deficit driven consolidation alone. Breuer (2019) adds a distinct reverse causality mechanism on the spending side, because rising output can mechanically reduce expenditure to GDP ratios when cyclical adjustment is incomplete. Broad consolidation evidence therefore cannot establish the dynamic causal effect of a specific expenditure instrument on output or debt.
...
| Izquierdo et al. (2019) | Public investment and initial public capital stock | Public investment: impact 0.15, two years 0.80; low initial public capital: two years 2.15; high initial public capital: two years 0.15. |
...
| Saccone et al. (2022) | Public investment in European countries | Total public investment: 0.979 on impact and 2.056 at horizon 6; economic affairs investment: 0.859 on impact and 2.336 at horizon 6. |
...
The remainder is organised as follows. Section 2 presents the data, fiscal shock construction, and local projection methodology. Section 3 reports the estimated response trajectories. Section 4 applies them to a public investment consolidation scenario for Poland. Section 5 concludes. The appendices build sequentially from raw inputs to final results: Appendix A defines the state variables and reports the diagnostic screening; Appendix B reports the full cumulative response paths; Appendix C reports the annual debt to GDP projection paths, including the symmetric expansion counterpart; and Appendix D details the underlying local projection coefficient estimates, standard errors, and p values.
...
Here $i$ indexes EU27 member states and $t$ indexes years. The dependent variable $y_{i,t+h}$ is the scaled outcome at horizon $h$: real GDP in output regressions and public investment spending in spending regressions. The output outcome at horizon $h$ is the change in real GDP between years $t+h-1$ and $t+h$, divided by real GDP in year $t-1$; at the impact horizon the change is measured from year $t-1$ to year $t$. The spending outcome applies the same construction to real public investment spending, with the same year $t-1$ real GDP denominator, so output and spending responses share one unit relative to GDP in the year before the shock. The cumulative responses in Section 3 sum these horizon specific increments. The term $s^{GI}_{i,t}$ is the identified public investment shock, while $s^{GC}_{i,t}$ is the identified public consumption shock included as a fiscal control. The lagged controls match the fiscal, output, and monetary variables used in the recursive shock identification system, following the practice of conditioning local projections on pre shock dynamics in the relevant information set (Jorda, 2005; Ciaffi, Deleidi and Di Domenico, 2024).
...
In short, the identified shocks enter horizon specific local projections as regressors. This two step structure, with shock identification first and local projections second, follows Ciaffi, Deleidi and Di Domenico (2024), and the present paper adapts it to an EU27 annual panel and to the evaluation of Polish state variables.
...
Household liquidity position is measured from financial accounts as the negative of household financial assets net of liabilities, divided by GDP. The numerator excludes non financial assets, including housing, and the measure is transformed so that higher values indicate a weaker liquidity position relative to GDP. The variable captures the household financial buffer channel, since fiscal shocks may propagate differently when households have limited net financial capacity to smooth consumption after income fluctuations. Kaplan, Violante and Weidner (2014) provide the microeconomic distinction between liquid resources and illiquid wealth that motivates treating liquid household resources as relevant for consumption smoothing. Bernardini, De Schryder and Peersman (2020) show that household leverage conditions the transmission of fiscal shocks through balance sheet positions. Krajewski and Pilat (2025) give the Polish household finance context in which liquidity constraints and financial buffers are empirically relevant. The EU27 measure used here is therefore a comparable macro financial accounts state. It does not measure liquid deposits directly at the household survey level.
...
Section 2.2 defined the four channels and the reasons for evaluating them individually. This subsection sets out the criterion that decides which channels are carried into the Polish scenario. In the finite annual EU27 panel, the design gives the coefficients a direct interpretation as distinct economic channels.

Each channel evaluation must meet several conditions: the local projection must be estimable, exhibit acceptable numerical stability, include Poland within the observed EU27 data range, have viable output and spending denominators, and show an eighth year output interaction at $p \leq 0.10$. The eighth year is used as the diagnostic horizon because it directly informs the cumulative response comparison and the debt translation.

The design applies empirical discipline to state dependent local projections. Relevant conditioning dimensions can matter for interaction effects, while estimating several interaction dimensions at once can weaken empirical support, produce unstable estimates, and place too much weight on a limited number of country year observations (Cloyne, Jorda and Taylor, 2023). The design therefore separates the a priori definition of the state variable set from the statistical decision to carry a channel into the debt scenario. The resulting estimates should be read as individual channel evaluations. They do not form a model of simultaneous interaction among all macroeconomic states.

Four individual channel evaluations are conducted. Investment import content, household liquidity position, and public debt each satisfy the eighth year output interaction criterion, with p values of 0.002, 0.003, and 0.080. Real GDP per capita in PPS does not meet this criterion, with a p value of 0.461, and is therefore excluded from the Polish scenario projections. Table 2b reports the full retention decision behind the primary response and subsequent debt analysis.
...
Table 2b. Full retention screen for Polish channel evaluations and joint diagnostic.

| Polish channel evaluation | Eighth year output interaction p value | Polish state profile used | Scenario status |
| --- | ---: | --- | --- |
...
Second, state dependence is evaluated through individual channels specified in advance rather than through a joint interaction surface. This maintains interpretability of the state space, but it also means that import leakage, fiscal position, household liquidity, and development level are evaluated as separate conditional paths. They are not estimated within a model in which all macroeconomic states interact simultaneously. Cloyne, Jorda and Taylor (2023) treat interaction structure as central to impulse response heterogeneity, while also warning that richer state spaces raise identification and interpretation demands. Goncalves, Herrera, Kilian and Pesavento (2023) show that state dependent local projections recover valid responses only under restrictions on how the conditioning state reacts to the shock, a further reason to favour slow moving predetermined states. In this finite EU27 annual panel, jointly estimating all interactions would weaken empirical discipline, increase instability, and place too much weight on limited country year observations. The reported paths should therefore be read as partial channel evaluations rather than as a complete interaction surface for fiscal transmission.
...
Fifth, the estimation does not separate public investment by financing source. Part of the public investment variation in the EU27 panel, especially in the countries receiving large cohesion transfers, is financed by EU funds rather than by national borrowing. The estimated responses therefore describe the average historical episode, in which some investment movements had only a partial direct counterpart in national deficits. The scenario application in Section 4 treats the one percentage point action as a fully nationally financed change in the discretionary primary balance. For investment lines financed in part by EU funds, the national budgetary saving from a cut would be smaller than one for one, while conditions attached to EU funding can tighten further under the excessive deficit procedure, as Section 1.1 notes. EU transfers are also ultimately financed by the member states themselves through their contributions to the EU budget. For a country that receives more from the EU budget than it contributes, part of the investment financed by EU funds does not draw down national resources in the same period. These offsetting contribution flows are spread over long periods and lie largely beyond the eight year estimation horizon used here.
...
The evaluation of household liquidity position for Poland yields an eighth year cumulative output response of 2.24. Here, Poland is evaluated through the household sector's net financial balance sheet position rather than a direct measure of credit flows. The state thus captures a financial accounts balance sheet channel. Kaplan, Violante and Weidner (2014) distinguish liquid resources from illiquid wealth in household consumption smoothing, while Bernardini, De Schryder and Peersman (2020) show that household leverage conditions the transmission of government spending shocks through balance sheet positions. Fiscal shocks can therefore propagate differently when household financial assets net of liabilities are comparatively weaker relative to GDP.
...
Figure 1 plots the full sequences. At each horizon, panel (a) reports $K_Y(h)$, the cumulative real GDP response under the public investment shock normalization used here, and panel (b) reports the corresponding cumulative spending response $K_G(h)$. Both panels include the EU27 benchmark, the three Polish debt scenario evaluations, and the equal weight Polish average.

![Cumulative output and spending responses from impact through the eighth year. Panel (a) plots the cumulative output response $K_Y(h)$; panel (b) plots the cumulative spending response $K_G(h)$ under the same shock normalisation. Both panels show the EU27 benchmark, the three retained Polish channel evaluations, and the equal weight Polish average.](figures/figure_ky_kg_paths.png)
...
Fiscal policy is evaluated here as a path rather than as a sequence of independent annual shocks. Three strands of the literature support this approach. First, cumulative and present value multipliers compare output responses with spending responses over a horizon, rather than only an impact coefficient (Ilzetzki, Mendoza and Vegh, 2013; Ramey and Zubairy, 2018; Ramey, 2019; Huidrom, Kose, Lim and Ohnsorge, 2019; Saccone et al., 2022). Second, fiscal plans are normally implemented over time, so their effects on budgets and GDP should be evaluated as paths rather than as isolated one year events (Alesina, Favero and Giavazzi, 2015; Ramey, 2019). Third, debt outcomes depend on how spending, output, the primary balance, and the debt to GDP denominator interact (Fatas and Summers, 2018; Ciaffi, Deleidi and Di Domenico, 2024). The debt application below uses a consolidation scenario: three annual public investment reductions of one percentage point of GDP in 2028, 2029, and 2030, combined with the estimated spending and output response paths. Appendix C also reports the symmetric expansion counterpart as a decomposition check.

The programmed annual action vector is
...
where $s=0,1,2$ correspond to 2028, 2029, and 2030. A single 2028 cut generates the spending path $P_G^{(1)}(h)=-K_G(h)$ and the output path $P_Y^{(1)}(h)=-K_Y(h)$. The three year programme then adds the delayed responses from the three programmed actions:
...
The 2031 spending effect is therefore $a_0K_G(3)+a_1K_G(2)+a_2K_G(1)$, with $a_3=0$. The nonzero value after 2030 is not a fourth annual fiscal action. It is the continuing dynamic response to the three actions already introduced. The one percentage point figure is the annual policy scale within a multiannual investment framework. The full dynamic spending trajectory follows from the estimated response paths.

The same estimated dynamics shape the fiscal savings themselves. Because the cumulative spending response declines from one at impact to about 0.75 in the eighth year, part of an initial investment reduction is reversed in later budgets in the average estimated episode. Across the reported paths, the three one percentage point actions therefore deliver a direct primary balance improvement of about 2.0 to 2.2 percentage points of GDP in 2036 rather than the full 3 percentage points; the annual decompositions in Appendix C report the exact path. Once the cyclical primary balance feedback through automatic stabilisers is included, the net budgetary improvement is confined to the three action years. In the equal weight average path, the scenario primary balance is above the baseline by 0.15 to 0.41 percentage points of GDP between 2028 and 2030 and falls below the baseline from 2031 onward, by about 0.4 percentage points of GDP in 2036 (Table C.k2). The debt calculations credit the scenario with exactly this estimated saving path, so the savings and the output losses enter the debt comparison from the same estimated response dynamics.

The cut changes the discretionary primary balance impulse and the estimated output response while keeping the accounting environment constant.
...
The second debt calculation directly estimates the debt to GDP local projection path. It assesses the debt to GDP response to the same public investment shock and applies this estimated response to the same three annual reductions. A cut initially improves the primary balance; however, the associated output loss and the resulting increase in the debt ratio response may still elevate the debt ratio at the endpoint. Debt dynamics and hysteresis models evaluate fiscal policy through their effects on output persistence and debt to GDP ratios (DeLong and Summers, 2012; Fatas and Summers, 2018). The direct debt calculation aligns with empirical research linking government investment shocks to public debt sustainability outcomes (Ciaffi, Deleidi and Di Domenico, 2024).


## 4.2 2036 debt to GDP impact across specifications

This subsection reports the 2036 debt to GDP margins for the public investment cut scenario defined in Section 4.1. Each table entry is a percentage point difference from the baseline debt to GDP ratio in 2036. Positive values mean the debt to GDP ratio is higher than baseline.
...
Table 5 reports the endpoint margins. Figure 2 traces the annual debt to GDP margins behind these endpoints. In both debt calculations, the cut scenario margin is positive in every year from 2028 to 2036: the debt ratio stays above the Commission baseline throughout the projection window, and the institutional debt equation margin widens towards the endpoint.

![Annual debt to GDP margins under the public investment cut scenario, 2028 to 2036. Positive values mean debt above the Commission baseline. Panel (a) reports the institutional debt equation; panel (b) reports the direct debt to GDP local projection path. The shaded band spans the three retained Polish channel evaluations.](figures/figure_debt_margin_paths.png)

\TableBlock

Table 5. 2036 debt to GDP margins relative to baseline under the public investment cut scenario.

| Empirical path | Cut, institutional debt equation | Cut, direct debt to GDP local projection path |
| --- | ---: | ---: |
| EU27 panel benchmark | 7.7 | 3.5 |
| Polish evaluation based on investment import content | 4.3 | 1.7 |
| Polish evaluation based on household liquidity position | 7.3 | 3.5 |
| Polish evaluation based on public debt | 3.7 | 2.7 |
| Equal weight average across the three retained Polish channel evaluations | 5.1 | 2.6 |

![2036 debt to GDP margins under the public investment cut scenario. Positive values mean debt above baseline; the figure reports the institutional debt equation and direct debt to GDP local projection path.](figures/figure_debt_margins_2036.png)

Figure 3 compares the two debt calculations at the 2036 endpoint for the same public investment cut. Because the baseline debt to GDP ratio rises over 2028-2036, the consolidation question is whether the cut leaves the debt ratio below or above this baseline by 2036. In every reported specification, it leaves the ratio above baseline. The endpoint pattern follows from output persistence, which enters the debt calculation directly: sustained output losses after investment cuts raise the debt to GDP ratio through the denominator and related budget feedbacks.

The EU27 panel benchmark establishes a comparative standard for the analysis. A three year public investment cut increases the 2036 debt to GDP ratio by 7.7 percentage points under the institutional debt equation and by 3.5 percentage points under the direct debt to GDP local projection path. These outcomes are measured at the 2036 endpoint, eight years after the first action and six years after the final action.

The three retained Polish channel evaluations keep the same sign while differing in magnitude. In the evaluation based on investment import content, the cut raises the 2036 debt to GDP ratio by 4.3 percentage points under the institutional debt equation and by 1.7 percentage points under the direct debt to GDP path. In the household liquidity position evaluation, the corresponding margins are 7.3 and 3.5 percentage points. In the public debt evaluation, they are 3.7 and 2.7 percentage points.

The equal weight average across the three Polish evaluations reinforces the individual specification results. The public investment cut raises the 2036 debt to GDP ratio by 5.1 percentage points under the institutional debt equation and by 2.6 percentage points under the direct debt to GDP local projection path. Among the retained Polish channel evaluations, cut margins therefore range from 3.7 to 7.3 percentage points under the institutional debt equation and from 1.7 to 3.5 percentage points under the direct debt to GDP path. The pattern is consistent with the self defeating consolidation literature: when output effects persist, the GDP channel can overturn the apparent fiscal gain from public investment cuts (DeLong and Summers, 2012; Fatas and Summers, 2018). Appendix C reports the symmetric expansion counterpart and the annual debt decomposition behind both directions.

# 5 Conclusion

The analysis examined public investment as both an instrument of development policy and an object of fiscal surveillance. The central 2036 endpoint result is that cutting public investment raises the debt to GDP ratio relative to the Commission baseline used here as the reference path. The debt application uses three annual investment cuts, each equal to 1 percentage point of GDP in 2028, 2029 and 2030. Across the three retained Polish channel evaluations, endpoint margins for cuts range from 3.7 to 7.3 percentage points under the institutional debt equation and from 1.7 to 3.5 percentage points under the direct debt to GDP local projection path.
...
These findings are relevant for EU fiscal surveillance because debt projections are sensitive to multiplier assumptions, output persistence and the composition of fiscal adjustments. A common short run multiplier can make investment cuts look more effective as a consolidation measure than they are once instrument specific output paths enter the calculation. When projected debt paths, expenditure ceilings and progress assessments shape national fiscal choices, empirically based output responses for public investment change the estimated debt outcome of consolidation measures substantially.
...
In Poland's context, the persistence of estimated output responses feeds directly into debt outcomes. Public investment should therefore not be treated as an easy budgetary margin against the Commission Debt Sustainability Monitor baseline used here as the reference path: in the reported cut scenario, reducing investment worsens the debt ratio once medium term output effects are carried into the debt calculation.
...
Bernardini, M., De Schryder, S. and Peersman, G. (2020). Heterogeneous government spending multipliers in the era surrounding the Great Recession. Review of Economics and Statistics, 102(2), 304-322.
...
Cugnasca, A. and Rother, P. (2015). Fiscal multipliers during consolidation: evidence from the European Union. ECB Working Paper No. 1863. European Central Bank.
...
Saccone, D., Della Posta, P., Marelli, E. and Signorelli, M. (2022). Public investment multipliers by functions of government: An empirical analysis for European countries. Structural Change and Economic Dynamics, 60, 531-545.
...
Van der Veer, R. A. (2022). Walking the tightrope: politicization and the Commission's enforcement of the SGP. JCMS: Journal of Common Market Studies, 60(1), 81-100.
...
The rank and condition number checks follow standard regression diagnostics (Welsch and Kuh, 1977).
...
with the support p value computed from the corresponding reference distribution. The support check guards against evaluating Poland outside the region where the panel provides overlapping state values (Crump, Hotz, Imbens and Mitnik, 2009). The output interaction at $h=8$ supplies the response relevance criterion:

$$
H_0:\theta_{8,c}=0.
$$

The Polish scenario carries the individual channel evaluations that satisfy the numerical diagnostics, support diagnostics, denominator diagnostics, and eighth year output interaction criterion of $p \leq 0.10$. In this mixed window run, investment import content, household liquidity position, and public debt satisfy the criterion and are retained. Real GDP per capita in PPS does not satisfy the criterion and is not retained in the scenario projections.

The joint four state diagnostic is not used as the main scenario path because its eighth year output interaction p value is 0.179, above the same 0.10 retention threshold. The scenario therefore uses the retained individual channel evaluations reported below.

\TableBlock

Table A.3. Channel screening and joint diagnostic by eighth year output interaction.
...
| Joint four state interaction | 0.179 | Criterion not satisfied | Diagnostic not retained in the Polish scenario |

Notes: The first four rows apply the statistical screening criterion to individual transmission channels. The joint row reports the diagnostic in which all four state interactions enter together. The equal weight Polish path in the main results averages the three retained channel evaluations: investment import content, household liquidity position, and public debt.
...
The figure below plots the ratio paths behind Table B.3 against the Commission reference value.
...
The figure below plots the two opposing forces in the cut scenario for the equal weight average path: the direct primary balance saving delivered by the estimated spending response and the output shortfall that drives the debt ratio above baseline.

![Direct primary balance saving and output shortfall under the public investment cut, equal weight average path, 2028 to 2036. The saving follows the estimated spending response and reaches about 2.1 percentage points of GDP in 2036; the output shortfall reaches about 5.1 percent of GDP relative to baseline.](figures/figure_saving_output_loss.png)
...
Notes to Tables C.b1 to C.k2: Output effects are reported relative to the baseline output level; negative values in the expansion case denote higher output relative to baseline in the debt accounting convention, while positive values in the cut case denote the corresponding output shortfall. Direct primary balance effects are the discretionary fiscal actions generated by the three year programme. Cyclical primary balance feedback records the endogenous budget response to the output movement. Institutional debt margins are the margins from the medium term debt equation; direct debt to GDP margins are the corresponding local projection debt ratio margins estimated for the same signed investment action.
...
The joint four state diagnostic estimates the corresponding specification with all candidate state variables entering together at the eighth year horizon:

$$
x_{i,t+8}^{m}
= \alpha_i^8 + \tau_t^8 + \beta_{J,8}^{m}s_{i,t}^{GI}
+ \sum_{c\in C}\theta_{c,8}^{m}(s_{i,t}^{GI}z_{c,i,t-1})
+ \sum_{c\in C}\lambda_{c,8}^{m}z_{c,i,t-1}
+ \gamma_{J,8}^{m}s_{i,t}^{GC}
+ \delta_{J,8}^{m\prime}W_{i,t-1}
+ u_{i,t+8}^{m},
$$

where $C$ contains investment import content, household liquidity position, public debt, and real GDP per capita in PPS. This diagnostic answers whether a single four state interaction surface should replace the individual retained Polish channel evaluations. It is estimated as a screening diagnostic only; it is not carried into the debt scenario because its joint eighth year output interaction p value is 0.179, above the retention threshold of 0.10.
...
Table D.1 reports the estimation setup. Table D.2 reports eighth year coefficient estimates and normalised response ratios for the linear benchmark and the four individual transmission channels. Table D.3 summarises pointwise p values by channel and outcome over horizons $h=0,\ldots,8$. Table D.4 reports the eighth year sample diagnostics and Poland profile values. Table D.5 shows how the retained response paths translate into cumulative $K_Y(8)$, $K_G(8)$, and $K_Y(8)/K_G(8)$ values. Tables D.6 and D.7 report the joint four state diagnostic requested by the channel retention discussion.

All standard errors and p values in this appendix come from a Driscoll and Kraay (1998) covariance estimator. The scores of the residualised regressors are summed within each year, the resulting annual score covariances enter a Bartlett kernel with a bandwidth equal to the response horizon, and the covariance carries small sample corrections for the number of years and the number of estimated parameters (Cameron and Miller, 2015). The standard error of each normalised response ratio applies the delta method to the joint covariance of the output coefficient and the spending scale coefficient under the same scheme, and the reported p values use the normal approximation. The joint four state diagnostic in Tables D.6 and D.7 uses the corresponding Wald statistic with a chi squared reference distribution. At the eighth year horizon these quantities rest on fourteen shock years, so they are large sample approximations in the time dimension; bootstrap methods designed for few clusters offer a more conservative alternative (MacKinnon, Nielsen and Webb, 2023).

Appendix D reports pointwise, horizon level diagnostics. Because local projections estimate separate coefficients by horizon, the standard errors and p values in these tables describe each displayed coefficient at that horizon. They are not simultaneous confidence bands, path level tests, or no effect tests for the full response sequence (Jorda, 2005, 2007; Montiel Olea and Plagborg-Moller, 2021; Inoue, Jorda and Kuersteiner, 2024; Barnichon and Brownlees, 2019; Li, Plagborg-Moller and Wolf, 2024). Related fiscal and public investment multiplier papers likewise report response paths with confidence bands, p values, or horizon specific significance indicators when precision varies over the horizon (Deleidi, Iafrate and Levrero, 2020; Ciaffi, Deleidi and Di Domenico, 2024; Saccone et al., 2022).
...
Table D.6. Joint four state diagnostic at the eighth year horizon.

| Diagnostic item | Value |
| --- | --- |
| Observations | 378 |
| Countries | 27 |
| Shock years | 2004-2017 |
| Regression rank | 14 |
| Regressor count | 14 |
| Condition number | 31.858 |
| Maximum absolute state correlation | 0.506 |
| Poland support p value | 0.961 |
| Maximum absolute Polish profile z value | 0.576 |
| Denominator t statistic | 42.425 |
| Joint output interaction p value | 0.179 |
| $K_Y(8)$ | 0.327 |
| $K_G(8)$ | 0.706 |
| $K_Y(8)/K_G(8)$ | 0.463 |
| Admission result | Not retained |

\TableBlock

Table D.7. Joint four state diagnostic interaction coefficients at the eighth year horizon.

| Interaction term | Coefficient | Standard error | p value |
| --- | ---: | ---: | ---: |
| Public investment shock x investment import content | -0.012703 | 0.007448 | 0.088 |
| Public investment shock x household liquidity position | 0.009193 | 0.009824 | 0.349 |
| Public investment shock x public debt | -0.006825 | 0.003771 | 0.070 |
| Public investment shock x real GDP per capita in PPS | 0.008870 | 0.005881 | 0.132 |

Notes to Tables D.1 to D.7: Parentheses in Table D.2 report the standard error for the displayed coefficient or response ratio; the pointwise, horizon level p value is reported in its own column. Non-TiVA specifications use 2025 observations where the required Eurostat variables are available, including Ireland where the needed fields exist. Specifications using investment import content use the official OECD TiVA source window through 2022. The cumulative response translation uses the same horizon specific response estimates to construct the output and spending paths used in the debt application. In Table D.6, the condition number and maximum absolute state correlation indicate the numerical behavior of the included regressors, while the support p value and the maximum absolute Polish profile z value describe the position of Poland's evaluated state profile within the EU27 distribution. In Table D.7, the displayed component p values are coefficient diagnostics; the retention criterion for the joint diagnostic is the joint output interaction p value of 0.179 reported in Table D.6.
